<template>
  <section
    ...
    :class="[{ 'section--hero--small': small }, { 'section--hero--shadow': !noshadow }, { 'section--orange': backgroundcolor === 'orange' }]"
  >
    <div ref="heroStatic" class="hero__static"/>
    <div v-if="slides && slides.length > 0" class="slick">
      <div v-for="slide in slides" :key="slide.id" class="slide">
        <div v-if="slide.background_image" ref="heroImage" class="hero__image">
        ...
        <div class="section--wrapper">
          <div ref="heroContent" class="hero__content">
            <div class="hero__content-text">
              ...
              <h1>
                {{ slide.title }}
                <small v-if="slide.sub_title">{{ slide.sub_title }}</small>
              </h1>
              <p v-if="slide.text">{{ slide.text }}</p>
              <div v-if="slide.link !== ''" class="cta cta--button">
                <a :href="slide.link.url" :target="slide.link.target">{{ slide.link.title }}</a>
              </div>
            </div>
          </div>
        </div>
